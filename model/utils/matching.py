...
def scale_correlation_softmax(feature0, feature1):
    ...
    feature0 = feature0.view(b, c, -1).permute(0, 2, 1).contiguous()  # [B, H*W, C]
    feature1 = feature1.view(b, c, -1)  # [B, C, H*W]
    ...
    grid = init_grid.view(b, 1, -1).permute(0, 2, 1).contiguous()  # [B, H*W, 1]
    ...
    correspondence = torch.matmul(prob, grid).view(b, h, w, 1).permute(0, 3, 1, 2).contiguous()  # [B, 1, H, W]
...
def global_correlation_softmax(feature0, feature1,
    ...
    # flow from softmax
    init_grid = coords_grid(b, h, w).to(correlation.device)  # [B, 2, H, W]
    grid = init_grid.view(b, 2, -1).permute(0, 2, 1).contiguous()  # [B, H*W, 2]

    correlation = correlation.view(b, h * w, h * w)  # [B, H*W, H*W]

    if pred_bidir_flow:
        correlation = torch.cat((correlation, correlation.permute(0, 2, 1).contiguous()), dim=0)  # [2*B, H*W, H*W]
    ...
    correspondence = torch.matmul(prob, grid).view(b, h, w, 2).permute(0, 3, 1, 2).contiguous()  # [B, 2, H, W]
...
def local_correlation_softmax(feature0, feature1, local_radius,
    ...
    window_feature = F.grid_sample(feature1, sample_coords_norm,
                                   padding_mode=padding_mode, align_corners=True
                                   ).permute(0, 2, 1, 3).contiguous()  # [B, H*W, C, (2R+1)^2]
