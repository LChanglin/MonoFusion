...
def single_head_full_attention(q, k, v):
    ...
    scores = torch.matmul(q, k.permute(0, 2, 1).contiguous()) / (q.size(2) ** .5)  # [B, L, L]
...
def single_head_full_attention_1d(q, k, v,
    ...
    scores = torch.matmul(q, k.permute(0, 1, 3, 2).contiguous()) / scale_factor  # [B, H, W, W]
...
def single_head_split_window_attention(q, k, v,
    ...
    scores = torch.matmul(q.view(b_new, -1, c), k.view(b_new, -1, c).permute(0, 2, 1).contiguous()
...
class SelfAttnPropagation(nn.Module):
    ...
    def forward(self, feature0, flow,
        ...
        query = feature0.view(b, c, h * w).permute(0, 2, 1).contiguous()  # [B, H*W, C]
        ...
        value = flow.view(b, flow.size(1), h * w).permute(0, 2, 1).contiguous()  # [B, H*W, 2]

        scores = torch.matmul(query, key.permute(0, 2, 1).contiguous()) / (c ** 0.5)  # [B, H*W, H*W]
        prob = torch.softmax(scores, dim=-1)

        out = torch.matmul(prob, value)  # [B, H*W, 2]
        out = out.view(b, h, w, value.size(-1)).permute(0, 3, 1, 2).contiguous()  # [B, 2, H, W]
    ...
    def forward_local_window_attn(self, feature0, flow,
        ...
        feature0_reshape = self.q_proj(feature0.view(b, c, -1).permute(0, 2, 1).contiguous()
                                       ).reshape(b * h * w, 1, c)  # [B*H*W, 1, C]

        kernel_size = 2 * local_window_radius + 1

        feature0_proj = self.k_proj(feature0.view(b, c, -1).permute(0, 2, 1)).permute(0, 2, 1).contiguous().reshape(b, c, h, w)
        ...
        feature0_window = feature0_window.view(b, c, kernel_size ** 2, h, w).permute(
            0, 3, 4, 1, 2).contiguous().reshape(b * h * w, c, kernel_size ** 2)  # [B*H*W, C, (2R+1)^2]
        ...
        flow_window = flow_window.view(b, value_channel, kernel_size ** 2, h, w).permute(
            0, 3, 4, 2, 1).contiguous().reshape(b * h * w, kernel_size ** 2, value_channel)  # [B*H*W, (2R+1)^2, 2]
