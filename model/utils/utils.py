...
def split_feature(feature,
    ...
    if channel_last:  # [B, H, W, C]
        ...
        feature = feature.view(b, num_splits, h // num_splits, num_splits, w // num_splits, c
                               ).permute(0, 1, 3, 2, 4, 5).contiguous().reshape(b_new, h_new, w_new, c)  # [B*K*K, H/K, W/K, C]
    else:  # [B, C, H, W]
        ...
        feature = feature.view(b, c, num_splits, h // num_splits, num_splits, w // num_splits
                               ).permute(0, 2, 4, 1, 3, 5).contiguous().reshape(b_new, c, h_new, w_new)  # [B*K*K, C, H/K, W/K]
...
def feature_add_position3(feature0, feature1, feature2, attn_splits, feature_channels):
...
def upsample_scale_with_mask(scale, up_mask, upsample_factor):
    ...
    up_scale = up_scale.permute(0, 1, 4, 2, 5, 3).contiguous()  # [B, 1, K, H, K, W]
