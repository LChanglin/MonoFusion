...
from .loss import get_loss, get_loss2, get_loss_multi, get_loss_test, get_loss_of, get_loss_multi_multi
...
class TTCTrainer(object):
    ...
    def train(self):
        ...
        loss_now, f1, epe = self.eval_epoch()
        print('Eval: ', loss_now, ' ', f1, ' ', epe)
        for epoch in range(self.start_epoch, self.epoch):
            ...
            if (self.parallel and torch.distributed.get_rank()==0) or (not self.parallel):
                ...
                file.write('Train %f %f %f '%(self.loss_sum_per_epoch/max(1,self.iters),\
                                             self.loss_per_epoch/max(1,self.iters), \
                                            self.f1/max(1,self.iters))) 
                # file.write('Train %f' %(self.loss_per_epoch/max(1,self.iters)))   
                file.write('Test %f %f %f\n'%(loss_now, f1, epe))          
                file.close()
    ...
    def train_epoch(self, epoch):
        ...
        for i, data in enumerate(self.train_loader):
            ...
            img0, img1, flow_gt, imgAux , valid, ts = data
            ...
            scale, flow, flow0 = self.model(img0, img1,
            ...
            if type(flow) == list:
                if type(scale) == list:
                    loss, loss_last, gt_scale, f1 = get_loss_multi_multi(scale, flow, imgAux, flow_gt, valid, epoch)
                else:
            ...
            if i%int(save_index)==0 :
                if not self.parallel or (self.parallel and torch.distributed.get_rank()==0):
                    ...
                    img = np.clip(img.detach().cpu().numpy(), 0.0, 255.0)
                    cv2.imwrite(os.path.join(out_dir, str(epoch)+'_'+str(i)+'img1'+'.jpg'), img)
                    img = ((img1[0]).transpose(0,1).transpose(1,2))
                    img = np.clip(img.detach().cpu().numpy(), 0.0, 255.0)
                    cv2.imwrite(os.path.join(out_dir, str(epoch)+'_'+str(i)+'img2'+'.jpg'), img)
            ...
            if i%(int(self.steps_per_epoch/5))==0 and i>0 and i<(self.steps_per_epoch-20):

                out_dir = "./log/%s_selfcon_ttc"%(self.time_stamp)
                loss_txt = out_dir + '/1.txt'
                loss_now, f1x, epe = self.eval_epoch()
                print('Eval: ', loss_now, ' ', f1x, ' ', epe)
                ...
                if (self.parallel and torch.distributed.get_rank()==0) or (not self.parallel):
                    ...
                    file.write('Train %f %f %f '%(self.loss_sum_per_epoch/max(1,self.iters),\
                                                ...
                                                self.f1/max(1,self.iters))) 
                    # file.write('Train %f' %(self.loss_per_epoch/max(1,self.iters)))   
                    file.write('Test %f %f %f '%(loss_now, f1x, epe))    
